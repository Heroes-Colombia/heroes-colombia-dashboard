...
export default function AdminBusinessAnalyticsPage() {
  ...
  const { promotions, isLoading: promotionsLoading } = usePromotions({ businessId })
  const { business, isLoading: businessLoading } = useBusiness(businessId)

  // Admin always sees enterprise level analytics
  ...
  useEffect(() => {
    async function fetchLocations() {
      ...
      try {
        const locs = await LocationService.getBusinessLocations(businessId)
        setLocations(locs || [])
      } catch (error) {
  ...
  useEffect(() => {
    async function fetchAnalyticsData() {
      ...
      try {
        const events = await AnalyticsEventService.getAnalyticsEvents(businessId, {
          dateRange,
        })
  ...
  // Calculate analytics - always show all levels for admin
  ...
  const locationAnalyticsData = useMemo(() => {
    if (filteredEvents.length > 0 && locations.length > 0) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">
              Analíticas: {business?.name || "Cargando..."}
            </h1>
            <p className="text-muted-foreground">
              Métricas detalladas de rendimiento del negocio
            </p>
          </div>
        </div>
        <Badge variant="default" className="flex items-center gap-1">
          <Crown className="h-3 w-3" />
          Vista Admin (Enterprise)
        </Badge>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Veces que aparece el negocio</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Veces que ven el perfil</p>
          </CardContent>
        </Card>
      ...
      {/* Advanced Analytics - Demographics (Admin sees all) */}
      ...
      {locationAnalyticsData.length > 0 && (
        <Card>
          <CardHeader>
            <div className="flex items-center gap-2">
